...
int do_extract_currentfile(unzFile uf, int opt_extract_without_path, int* popt_overwrite, const char *password, const char *destination)
{
    ...
    *popt_overwrite = 1;
    ...
    if (err != UNZ_OK)
    {
        ...
        snprintf (path, sizeof(path), "%s", out_dir);
    ...
    if ((err == UNZ_OK)){
     ...
     sprintf (path, "%s/%s", (const char*)out_dir, (const char*)write_filename);
     fout= fopen( (const char*)write_filename, "wb");

       if ((fout == NULL) && (filename_withoutpath != (char*)filename_inzip))
         {
        char c = *(filename_withoutpath-1);
        *(filename_withoutpath-1) = 0;
        makedir((const char*)write_filename, (const char*)out_dir);
        *(filename_withoutpath-1) = c;
        fout= fopen( (const char*)path, "wb");
          }
    ...
    if (err == 0)  {
    sprintf (path, "%s/%s", (const char*)out_dir, (const char*)write_filename);
    change_file_date((const char*)path, pfile_info.dosDate, pfile_info.tmu_date); }
...
int do_extract_all(unzFile uf, int opt_extract_without_path, int opt_overwrite, const char *password, const char *destination)
{
    ...
    do
    {
        err = do_extract_currentfile(uf, opt_extract_without_path, &opt_overwrite, password ,destination);
        if (err != UNZ_OK)
            break;
...
int makedir(const char *newdir, const char *destination)
{
    ...
    sprintf (path, "%s/%s", (const char*)out_dir, (const char*)newdir);

    int len = (int)strlen((const char*)path);
    ...
    int n2;
	for( n2=1;n2<255;n2++){
		if (buffer[len-n2] == '.'){ buffer[len-n2] = 0;  break;}
	}
...
/* Additions by MussoNero '2014 */
int
zip_unpack (const char *zipfilename, const char *destination)
{
    ...
    ret = do_extract_all(uf, opt_do_extract_withoutpath, opt_overwrite, password, out_dir);
    unzClose(uf);
    return ret;
