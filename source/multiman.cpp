...
static u32 new_pad_t[7];
static u32 old_pad_t[7];
static uint8_t old_status=0;
static uint8_t old_status_k=0;
static u8 pad_num=0;
static u8 active_pads=0;
static int last_pad=-1;
static u32 old_info[7];
static CellPadActParam _CellPadActParam[1];
static bool use_motor=false;

static int png_w=0, png_h=0, png_w2, png_h2;
static int jpg_w=0, jpg_h=0;
static int png_w_th=0, png_h_th=0;
...
char current_version[9]="O.D.E";
char current_version_NULL[10];
char versionUP[64];

char hdd_folder[64]="/dev_hdd0/game/BCED01063DATA/USRDIR/PS3GAMES_BACKUPS";

bool first_launch=1;
char app_path[32];
char app_temp[128];
char app_usrdir[64];
char app_usrdir_data[64];
char app_homedir[64];
...
char new_game_name[1024];
char new_game_path[1024];
char new_file_test[1024];

char path_1[1024]; char path_2[1024]; char path_3[1024]; char path_4[1024];
...
static u32 type_dialog_wait = CELL_MSGDIALOG_TYPE_SE_TYPE_NORMAL | CELL_MSGDIALOG_TYPE_BG_VISIBLE | CELL_MSGDIALOG_TYPE_BUTTON_TYPE_NONE | CELL_MSGDIALOG_TYPE_DISABLE_CANCEL_ON;
...
static u8 read_pad_info_browse()
{
...
static void add_browse_column()
{
	if(is_browse_loading || xmb[0].init || is_video_loading || is_music_loading || is_photo_loading || is_retro_loading || is_game_loading)
		return;
...
{
	if(is_video_loading || xmb[5].init || !xmb[6].init || is_game_loading || is_music_loading || is_photo_loading || is_retro_loading)
		return;
	is_video_loading=1;
	xmb[5].init=1;
...
{
	if(xmb[4].init || is_video_loading || is_music_loading || is_photo_loading || is_retro_loading || is_game_loading)
		return;
	is_music_loading=1;
	xmb[4].init=1;
...
{
	if(xmb[3].init || is_video_loading || is_music_loading || is_photo_loading || is_retro_loading || is_game_loading)
		return;
	is_photo_loading=1;
	xmb[3].init=1;
...
static void add_xmb_member(xmbmem *_member, u16 *_size, char *_name, char *_subname,
		/*type*/u8 _type, /*status*/u8 _status, /*game_id*/int _game_id, /*icon*/u8 *_data, u16 _iconw, u16 _iconh, /*f_path*/char *_file_path, /*i_path*/ char *_icon_path, int _u_flags, int _split, int _iso_num )
{
...
static void delete_xmb_member(xmbmem *_xmb, u16 *max, int n)
{
...
static void sort_xmb_col(xmbmem *_xmb, u16 max, const int first)
{
	if((max)<3)
		return;
	int n,m,o1=0,o2=0;
...
static u8 get_psx_region(char *_root)
{
	u8 region=0;
	int dir_fd;
	uint64_t nread;
...
static void download_cover(char *title_id, char *name)
{
...
static int ps3_home_scan_bare(char *path, t_dir_pane_bare *list, int *max)
{
...
static void add_game_column(t_menu_list *list, int max, int sel, bool force_covers)
{
		int toff=0;
		...
		if(cover_mode == MODE_XMMB)
		{
			...
			if(!xmb[5].init || !xmb[5].size)
			{
				...
				add_xmb_member(xmb[5].member, &xmb[5].size, (char*)STR_XC5_LINK, (char*)STR_XC5_LINK1,
						/*type*/6, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_showtime, 128, 128, /*f_path*/(char*)"/", /*i_path*/(char*)"/", 0, 0, 0);
				...
				if(ss_patched && exist((char*)"/dev_bdvd"))
				{
					...
					add_xmb_member(xmb[5].member, &xmb[5].size, (char*)"BD/DVD", (char*)STR_SIDE_BROW,
							/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)"/dev_bdvd", /*i_path*/(char*)"/", 0, 0, 0);
				}
				...
				if(exist((char*)"/dev_sd"))
				{
					...
					add_xmb_member(xmb[5].member, &xmb[5].size, (char*)"SD/SDHC", (char*)"ROOT",
							/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)"/dev_sd", /*i_path*/(char*)"/", 0, 0, 0);
				}
				...
				if(exist((char*)"/dev_cf"))
				{
					...
					add_xmb_member(xmb[5].member, &xmb[5].size, (char*)"CF", (char*)"ROOT",
							/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)"/dev_cf", /*i_path*/(char*)"/", 0, 0, 0);
				}
				...
				if(exist((char*)"/dev_ms"))
				{
					...
					add_xmb_member(xmb[5].member, &xmb[5].size, (char*)"MS/MSPRO", (char*)"ROOT",
							/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)"/dev_ms", /*i_path*/(char*)"/", 0, 0, 0);
				}

				ext_devs++;
				add_xmb_member(xmb[5].member, &xmb[5].size, (char*)STR_BR_HDD, (char*)STR_SIDE_BROW,
						/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)"/dev_hdd0", /*i_path*/(char*)"/", 0, 0, 0);
				...
				for(int ret_f=0; ret_f<99; ret_f++)
				{
					...
					if(exist(icon_path))
					{
						...
						add_xmb_member(xmb[5].member, &xmb[5].size, t_ip, (char*)STR_SIDE_BROW,
								/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)icon_path, /*i_path*/(char*)"/", 0, 0, 0);
					}
				}
				...
				if(pane!=NULL)
				{
					...
					if( (exist((char*)"/dev_bdvd/SYSTEM.CNF") || exist((char*)"/dev_bdvd/system.cnf") || disc_in_tray==PSX_DISC || disc_in_tray==PS2_DISC))// && ss_patched
					{
						...
						if(disc_in_tray==PSX_DISC)
						{
							...
							/*type*/36, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_psx, 128, 128, /*f_path*/(char*)"/dev_bdvd", /*i_path*/(char*)"/", 0, 0, 0);
							disc_in_tray=PSX_DISC;
						}
						else
						{
					...
					if( (disc_in_tray==UNK_DISC || disc_in_tray==-1) && !hide_bd) // && c_firmware==3.55f
					{
						...
						add_xmb_member(xmb[6].member, &xmb[6].size, (char*)psx_title, (char*)"PSX",
						/*type*/36, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_blend, 128, 128, /*f_path*/(char*)"/dev_bdvd", /*i_path*/(char*)"/", 0, 0, 0);
						//disc_in_tray=PSX_DISC;
					}
		...
		add_mem_disc:
								...
								/*type*/1, /*status*/g_status, /*game_id*/m, /*icon*/g_icon, g_iconw, g_iconh, /*f_path*/list[m].path, /*i_path*/(char*)icon_path, list[m].user, list[m].split, 0);
								ext_devs++;
							}
						}
					}

					ps3_home_scan_bare(iso_ps3_cobra_path, pane, &max_dir);
					...
					cobra_manager_init(&manager);
									for(int i=0; i<manager.num_iso;  i++)
									{
										...
										if (manager.iso[i].sfo_path == NULL &&  manager.iso[i].type == 0)
										{
											...
											g_iconw=320; g_iconh=176;
											g_status=0;
											snprintf(linkfile, 511, "%s/%s", "/dev_bdvd/COBRA", manager.iso[0].filename);
											if(strlen(linkfile)>(sizeof(xmb[5].member[0].file_path)-1)) continue;
											if (manager.iso[0].type == 0)sprintf(icon_path, "/dev_hdd0/game/BCED01063DATA/USRDIR/DATA/COBRA.PNG"); g_status=0; g_icon=xmb_icon_blu; g_iconw=320; g_iconh=176;
											add_xmb_member(xmb[6].member, &xmb[6].size, (char*)manager.iso[0].filename, /*(char*)cobra_path,*/(char*)"Cobra PST",  /*typeiso_type*/99, /*status*/g_status, /*game_id*/0,  /*icon*/xmb_icon_blu, 320, 176, /*f_path*/linkfile, /*i_path*/(char*)"/dev_hdd0/game/BCED01063DATA/USRDIR/DATA/COBRA.PNG", 0, 0, i);
									...
									if (manager.iso[i].sfo_path != NULL && manager.iso[i].type == 3)
										{
											...
											//bluray_game[0]=0;
											int plevel=0;
											sprintf(sfo_file, "%s",(char*)manager.iso[i].sfo_path);
											parse_param_sfo(sfo_file, list[i].title, list[i].title_id, &plevel);
										   if(xmb_cover_column || force_covers)
											{
												sprintf(icon_path, "%s/%s.JPG", covers_dir, (char*)manager.iso[i].game_id);
												if(exist(icon_path)) {g_iconw=260; g_iconh=300; goto add_mm_cobra;}
												sprintf(icon_path, "%s/%s.PNG", covers_dir, (char*)manager.iso[i].game_id);
												if(exist(icon_path)) {g_iconw=260; g_iconh=300; goto add_mm_cobra;}
											}

										   if(list[xmb[xmb_icon].member[xmb[6].size].game_id].cover!=-1 && list[xmb[xmb_icon].member[xmb[6].size].game_id].cover!=1)
											 {
												sprintf(icon_path, "%s/%s.JPG", covers_dir, list[i].title_id);
												download_cover(list[xmb[xmb_icon].member[xmb[6].size].game_id].title_id, icon_path);
												list[xmb[xmb_icon].member[xmb[6].size].game_id].cover=1;
											 }
											sprintf(icon_path, "%s", (char*)manager.iso[i].png_path);
											if(exist(icon_path)) {g_iconw=320; g_iconh=176; goto add_mm_cobra;}
											sprintf(icon_path, "%s/%s_320.PNG", cache_dir, (char*)manager.iso[i].game_id);
											if(exist(icon_path)) {g_iconw=320; g_iconh=176; goto add_mm_cobra;}
											else {g_status=2; g_iconw=128; g_iconh=128;}

add_mm_cobra:
											snprintf(linkfile, 511, "%s/%s", "/dev_bdvd/COBRA", manager.iso[i].filename);
											if(strlen(linkfile)>(sizeof(xmb[5].member[0].file_path)-1)) continue;

											add_xmb_member(xmb[6].member, &xmb[6].size, list[i].title, (char*)"PS3 ISO",
												/*type*/32, /*status*/g_status, /*game_id*/i, /*icon*/xmb_icon_blu, g_iconw, g_iconh, /*f_path*/linkfile, /*i_path*/(char*)manager.iso[i].png_path, 0, 0, i);
...
add_mem:
						...
						/*type*/1, /*status*/g_status, /*game_id*/m, /*icon*/g_icon, g_iconw, g_iconh, /*f_path*/list[m].path, /*i_path*/(char*)icon_path, list[m].user, list[m].split, m);
				...
				else if(!strcmp(list[m].content, "AVCHD") || !strcmp(list[m].content, "BDMV") || !strcmp(list[m].content, "DVD"))
				{
					if(cover_mode == MODE_XMMB) //!xmb[5].init &&
					{
						...
						if(exist(t_ip))
							...
							/*type*/2, /*status*/0, /*game_id*/m, /*icon*/xmb_icon_film, 128, 128, /*f_path*/list[m].path, /*i_path*/(char*) t_ip, 0, 0, 0);
						else
							add_xmb_member(xmb[5].member, &xmb[5].size, list[m].title+toff, list[m].content,
							/*type*/2, /*status*/2, /*game_id*/m, /*icon*/xmb_icon_film, 128, 128, /*f_path*/list[m].path, /*i_path*/(char*)"/", 0, 0, 0);
					}
					else
					{
						if(cover_mode != MODE_XMMB)
						{
							...
							sprintf(t_ip, "%s/HDAVCTN/BDMT_O1.jpg", list[m].path);
							sprintf(app_usrdir_data, "/dev_hdd0/game/BCED01063DATA/USRDIR/DATA");
							if(!exist(t_ip)) sprintf(t_ip, "%s/BDMV/META/DL/HDAVCTN_O1.jpg", list[m].path);
							if(!exist(t_ip)) sprintf(t_ip, "%s/POSTER.JPG", list[m].path);
							if(!exist(t_ip)) sprintf(t_ip, "%s/COVER.JPG", list[m].path);
							if(!exist(t_ip)) sprintf(t_ip, "%s/NOID.JPG", app_usrdir_data);
							if(exist(t_ip))
...
static void add_emulator_column()
{
	mod_xmb_member(xmb[8].member, 0, (char*)STR_XC1_REFRESH, (char*)STR_XC1_REFRESH3);
	redraw_column_texts(8);
	if(is_video_loading || is_music_loading || is_photo_loading || is_retro_loading || is_game_loading || xmb[8].init) return;
	is_retro_loading=1;
...
static void draw_xmb_legend(const int _xmb_icon)
{
	if(xmb_bg_counter>30 || _xmb_icon==1 || _xmb_icon==9 || c_opacity2<=0x40 || xmb_slide_step!=0 || xmb_slide_step_y!=0 || xmb_popup==0 || key_repeat)
		return;
...
static void gray_texture( uint8_t *buffer_to, uint32_t width, uint32_t height, int step)
{
	if(gray_poster==0)
		return;
	uint32_t cline=0;
...
static int load_jpg_texture(u8 *data, char *name, uint16_t _DW)
{
	scale_icon_h=0;
...
static void draw_xmb_title(u8 *buffer, xmbmem *member, int cn, u32 col1, u32 col2, u8 _xmb_col)
{
...
static int find_free_buffer(const int _col)
{
	(void) _col;
	int n;
...
static void draw_xmb_icons(xmb_def *_xmb, const int _xmb_icon_, int _xmb_x_offset, int _xmb_y_offset, const bool _recursive, int sub_level, int _bounce)
{
...
static void draw_whole_xmb(u8 mode)
{
	char filename[1024];
		...
		if(xmb_bg_counter==0 && !xmb_bg_show && xmb_slide_step==0 && xmb_slide_step_y==0 && (xmb_game_bg==1 || xmb_cover==1) && (xmb_icon!=6 || (xmb_icon==6 && xmb[xmb_icon].first)) && !is_game_loading)
		{
			...
			if(xmb_cover==1)// && xmb_icon==6)
			{
				...
				else
				{
					...
					if(exist(filename))
						{
							sprintf(cvstr, "%s/CBOX3.PNG", app_usrdir_data);
							if(exist(cvstr)){
							...
							sprintf(filename, "%s/GLC.PNG", app_usrdir_data);
							if(exist(filename))
							{
...
static void wait_dialog_simple()
{
...
static void screen_saver()
{
		...
		if(use_drops)
		{
			sprintf(auraBG, "%s/DROPS.PNG", app_usrdir_data);
			if(exist(auraBG))
...
static void mountpoints(u8 _mode)
{
	if(_mode==MOUNT)
	{
...
static void launch_ps1_emu(u8 _mode)
{
			...
			if(strcmp(net_path, path2)==0 && strstr(path_new, "/ps3_home")==NULL)
			{
				...
				if(strcmp(temp, title)==0) {
					...
					for(n=0;n<pl;n++)
					{
						...
						{
							cellFsMkdir(tempname, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(tempname, 0777);
						}
					}
				}
			}
...
static void mount_with_cache(const char *path, int _joined, u32 flags, const char *title_id)
{
	...
	if(strstr(path, "/dev_usb")!=NULL || (flags & IS_BDMIRROR))
	{
		...
		if((flags & IS_EXTGD))
		{
			...
			cellFsMkdir(ext_gd_path, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(ext_gd_path, 0777);
		}
	}
	else
	{
...
static void unknown_mimetype_callback(const char* mimetype, const char* url, void* usrdata)
{
	...
	cellFsMkdir(local_file_d, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(local_file_d, 0777);
...
static int open_select_menu(char *_caption, int _width, t_opt_list *list, int _max, u8 *buffer, int _max_entries, int _centered)
{
	...
	sprintf(filename, "%s/LBOX.PNG", app_usrdir_data);
...
reload_submenu:
	...
	if(!exist(label)) sprintf(label, "%s/NOID.JPG", app_usrdir_data); else goto gs_cover;
	if(exist(label)){
gs_cover:
		...
		sprintf(label, "%s/GLC.PNG", app_usrdir_data);
		if(exist(label))
		{
...
static double get_system_version(void)
{
	...
	if (fp != NULL) {
		...
		if(crc==0x41655eULL) base=3.55f; else
		base=4.55f;
...
void fill_entries_from_device(const char *path, t_menu_list *list, int *max, u32 flag, int sel)
{
	...
	if(first_launch) {sprintf(string2, "%s/PRB.PNG", app_usrdir_data);
...
void cache_png(char *path, char *title_id)
{
	...
	if(strstr (title_id, "AVCHD")!=NULL || strstr (title_id,"NO_ID")!=NULL) sprintf(src, "%s/BOOT.PNG", app_usrdir_data); else sprintf(src, "%s", path);
	cellFsMkdir(cache_dir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
	...
	else
	{
		if(strstr(src, "/PIC1.PNG")!=NULL && !exist(src))
		{
			...
			else
			{
				...
				{
					//						goto just_leave;
					sprintf(src, "%s/AVCHD.JPG", app_usrdir_data);
...
static int _my_game_copy_pfsm(char *path, char *path2)
{
	...
	do {
		...
		if ((entry.FileAttributes & PFS_FIND_DIR)) {
			...
			cellFsMkdir(d2f, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
...
static int _my_game_copy(char *path, char *path2)
{
	...
	while(1)
	{
		...
		if((entry->d_type & DT_DIR))
		{
			...
			cellFsMkdir(path2, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
			cellFsMkdir(d2f, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		...
		else
		{
			...
			if(use_symlinks==1)
			{
				if(strstr(d1, "/dev_hdd0/game/")!=NULL && strstr(d2, "/dev_hdd0/game/BCED01063DATA/USRDIR/GAMES/")!=NULL)
				{
...
pass_ok_2:
		...
		if(exist(filename)) {
				sprintf(usb_save, "/dev_sd/PRIVATE");
				 if(!exist(usb_save)) cellFsMkdir(usb_save, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		}
	...
	if(exist(usb_save)) {
		...
		sprintf(filename, "%s/AVCHD", usb_save); if(!exist(filename)) cellFsMkdir(filename, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		sprintf(filename, "%s/AVCHD/BDMV", usb_save); if(!exist(filename)) cellFsMkdir(filename, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
...
retry_showtime:
						...
						sprintf(filename, "/dev_bdvd/PS3_GAME/USRDIR/SHOWTIME.SELF");
						if(exist(filename)){goto showtime_exist;}
...
//						sprintf(other_pane,"%s/%s", temp_pane, just_title_id);
						...
						sprintf(other_pane,"/dev_hdd0/game/BCED01063DATA/USRDIR/GAMES");
						cellFsMkdir(other_pane, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(other_pane, 0777);
						...
						sprintf(other_pane,"/dev_hdd0/game/BCED01063DATA/USRDIR/GAMES/%s", just_title_id2);
						cellFsMkdir(other_pane, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(other_pane, 0777);
						my_game_copy((char *) list[m_copy].path, (char *) other_pane);
						...
						sprintf(other_pane, "/dev_hdd0/game/BCED01063DATA/USRDIR/GAMES/%s/USRDIR/EBOOT.BIN", just_title_id2);
						if(exist(filename)) {
							unlink(other_pane);
							remove(other_pane);
					...
					else
					{
						...
						else
						{
							...
							if(strstr("/ps3_home", tmp_path)!=NULL) {cellFsMkdir(other_pane, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(other_pane, 0777);}
...
//fm create iso
				...
				if( (xmb[xmb0_icon].member[xmb[xmb0_icon].first].type==35 || disc_in_tray==PS2_DISC) )
				{
					sprintf(filename, "%s/%sPS2.iso", other_pane, prefix);
				}
				else
				if( (xmb[xmb0_icon].member[xmb[xmb0_icon].first].type==36 || disc_in_tray==PSX_DISC) )
				{
...
//		}
		...
		if(!exist(usb_save) && payload>-1)
		{
			...
			cellFsMkdir(usb_save, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		}
...
void draw_xmb_bare(u8 _xmb_icon, u8 _all_icons, bool recursive, int _sub_level)
{
	...
	if(_all_icons==1)
		draw_xmb_icons(xmb, _xmb_icon, xmb_slide, xmb_slide_y, recursive, _sub_level, 0);
	else if(_all_icons==2)
		draw_xmb_icons(xmb, _xmb_icon, xmb_slide, xmb_slide_y, recursive, -1, 0);
	flip();
}
...
void apply_theme (const char *theme_file, const char *theme_path)
{
	...
	if(V_WIDTH>1280)
		sprintf(filename, "%s/MP_HR.PNG", app_usrdir_data);
	else
	{
		sprintf(filename, "%s/MP_LR.PNG", app_usrdir_data);
...
int open_theme_menu(char *_caption, int _width, theme_def *list, int _max, int _x, int _y, int _max_entries, int _centered)
{
	...
	load_texture(text_LIST+1512000, filename, 600);
	change_opacity(text_LIST+1512000, 50, 600*630*4);
	sprintf(filename, "%s/LBOX2.PNG", app_usrdir_data);
	...
	while(1)
	{
		...
		if(last_sel!=sel)
		{
			...
			sprintf(tdl, "SonicMan-Cobra-ODE version: %s", list[sel].mmver);
...
ve_not_ok:
				if(to_add)
				{
					...
					add_xmb_member(xmb[0].member, &xmb[0].size, (e_name+skip_first), e_entry,
							/*type*/e_type, /*status*/e_status, /*game_id*/-1, /*icon*/e_icon, 128, 128, /*f_path*/(char*)e_path, /*i_path*/(char*)icon_path, 0, 0, 0);
		...
		if(!xmb[0].size)
		{
			add_xmb_member(xmb[0].member, &xmb[0].size, (char*)no_titles, (char*) " ",
					/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_quit, 128, 128, /*f_path*/(char*)"/", /*i_path*/(char*)"/", 0, 0, 0);
		}
...
thumb_ok:
					...
					if(strstr(linkfile, iso_dvd_usb)!=NULL)
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (char*)"DVD ISO",
								/*type*/33, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_dvd, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
					else if(strstr(linkfile, iso_bdv_usb)!=NULL)
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (char*)"BD ISO",
								/*type*/34, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_bdv, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
					else
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (strstr(linkfile, "/dev_hdd0")==NULL ? (char*) "Video" : (char*)"HDD Video"),
								/*type*/3, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_film, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
...
thumb_not_ok:
					if(strstr(linkfile, iso_dvd_usb)!=NULL)
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (char*)"DVD ISO",
								/*type*/33, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_dvd, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
					else if(strstr(linkfile, iso_bdv_usb)!=NULL)
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (char*)"BD ISO",
								/*type*/34, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_bdv, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
					else
						add_xmb_member(xmb[5].member, &xmb[5].size, pane[ret_f].name, (strstr(linkfile, "/dev_hdd0")==NULL ? (char*) "Video" : (char*)"HDD Video"),
								/*type*/3, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_film, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
	...
	if(init_finished && is_photo_loading && xmb[3].init)
	{
		...
		for(int ret_f=0; ret_f<99; ret_f++)
		{
			...
			if(exist(linkfile))
			{
				...
				add_xmb_member(xmb[3].member, &xmb[3].size, tempstr, (char*)STR_SIDE_BROW,
					/*type*/0, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_usb, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
			}
		}
		...
		for(int ret_f=0; ret_f<max_dir; ret_f++)
		{
			if(strstr(pane[ret_f].name, ".jpg")!=NULL || strstr(pane[ret_f].name, ".JPG")!=NULL || strstr(pane[ret_f].name, ".png")!=NULL || strstr(pane[ret_f].name, ".PNG")!=NULL)
			{
				...
				/*type*/5, /*status*/i_status, /*game_id*/-1, /*icon*/xmb_icon_photo, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)t_ip, 0, 0, 0);
				...
				if(strstr(pane[ret_f].name, ".mp3")!=NULL || strstr(pane[ret_f].name, ".MP3")!=NULL || strstr(pane[ret_f].name, ".FLAC")!=NULL || strstr(pane[ret_f].name, ".flac")!=NULL)
				{
					...
					if(strlen(linkfile)<sizeof(xmb[4].member[0].icon_path)-1 && (strstr(linkfile, ".mp3")!=NULL || strstr(linkfile, ".MP3")!=NULL))
						add_xmb_member(xmb[4].member, &xmb[4].size, pane[ret_f].name+skip_first, (strstr(linkfile, "/dev_hdd0")==NULL ? (char*) "Music" : (char*)"HDD Music"),
								/*type*/4, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_note, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)linkfile, 0, 0, 0);
					else
						add_xmb_member(xmb[4].member, &xmb[4].size, pane[ret_f].name+skip_first, (strstr(linkfile, "/dev_hdd0")==NULL ? (char*) "Music" : (char*)"HDD Music"),
								/*type*/4, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_note, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
...
thumb_ok_iso:
					if(strstr(linkfile, iso_psx_usb)!=NULL)
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"PSX",
								/*type*/13, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_psx, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
					else if(strstr(linkfile, iso_ps2_usb)!=NULL)
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"PS2",
								/*type*/14, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_ps2, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
					else continue;
...
thumb_not_ok_iso:
					if(strstr(linkfile, iso_psx_usb)!=NULL)
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"PSX",
								/*type*/13, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_psx, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
					else if(strstr(linkfile, iso_ps2_usb)!=NULL)
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"PS2",
								/*type*/14, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_ps2, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
					else continue;
...
thumb_ok_snes:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"SNES9x Game ROM",
								/*type*/8, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
						goto thumb_cont_snes;

thumb_not_ok_snes:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"SNES9x Game ROM",
								/*type*/8, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
...
thumb_ok_genp:
						...
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"Genesis+ GX Game ROM",
								/*type*/11, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
						goto thumb_cont_genp;

thumb_not_ok_genp:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"Genesis+ GX Game ROM",
								/*type*/11, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
...
thumb_ok_fceu:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"Genesis+ GX Game ROM",
								/*type*/9, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
						goto thumb_cont_fceu;

thumb_not_ok_fceu:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"Genesis+ GX Game ROM",
								/*type*/9, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
thumb_cont_fceu:
						if(!(xmb[8].size & 0x0f)) sort_xmb_col(xmb[8].member, xmb[8].size, 1+ext_devs);
						if(xmb[8].size>=MAX_XMB_MEMBERS-1) break;
...
thumb_ok_vba:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"GameBoy Game ROM",
								/*type*/10, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
						goto thumb_cont_vba;

thumb_not_ok_vba:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"GameBoy Game ROM",
								/*type*/10, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
...
thumb_ok_fba:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"FBA Game ROM",
								/*type*/12, /*status*/0, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)imgfile2, 0, 0, 0);
						goto thumb_cont_fba;

thumb_not_ok_fba:
						add_xmb_member(xmb[8].member, &xmb[8].size, pane[ret_f].name, (char*)"FBA Game ROM",
								/*type*/12, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_retro, 128, 128, /*f_path*/(char*)linkfile, /*i_path*/(char*)"/", 0, 0, 0);
thumb_cont_fba:
						if(!(xmb[8].size & 0x0f)) sort_xmb_col(xmb[8].member, xmb[8].size, 1+ext_devs);
						if(xmb[8].size>=MAX_XMB_MEMBERS-1) break;
					}
...
//				/*type*/6, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_tool, 128, 128, /*f_path*/(char*)"/", /*i_path*/(char*)"/", 0, 0, 0);
	...
	if(settings_advanced)
	{
		...
		add_xmb_option(xmb[2].member, &xmb[2].size, (char*)STR_XC2_EMU, (char*)STR_XC2_EMU1,	(char*)"bd_emulator_Cobra-ODE");
		...
		add_xmb_suboption(xmb[2].member[xmb[2].size-1].option, &xmb[2].member[xmb[2].size-1].option_size, 0, (char*)"Cobra-ODE",		(char*)"1");
...
void add_home_column()
{
		...
		add_xmb_member(xmb[1].member, &xmb[1].size, (char*)STR_XC1_SS, (char*)STR_XC1_SS1,
				/*type*/6, /*status*/2, /*game_id*/-1, /*icon*/xmb_icon_ss, 128, 128, /*f_path*/(char*)"/", /*i_path*/(char*)"/", 0, 0, 0);
...
void init_xmb_icons(t_menu_list *list, int max, int sel)
{
		...
		sprintf(auraBG, "%s/MMLOGO1.PNG", app_usrdir_data); if(exist(auraBG)) load_texture(xmb_icon_logo, auraBG, 408);
		sprintf(auraBG, "%s/ARROW.PNG", app_usrdir_data); load_texture(xmb_icon_arrow, auraBG, 30);
...
{
	char reload_self[512];
	//sprintf(reload_self, "%s/RELOAD.SELF", app_usrdir);
	sprintf(reload_self, "/dev_bdvd/PS3_GAME/USRDIR/EBOOT.BIN");
	...
	if(exist(reload_self) /*&& net_used_ignore()*/)
	{
		...
		if(dialog_ret==1){
			...
			unload_modules();
			exitspawn((const char*) reload_self, NULL, NULL, NULL, 0, 1200, SYS_PROCESS_PRIMARY_STACK_SIZE_1M);
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	app_path[13]=0;
		...
		if(!strncmp( argv[0], "/dev_hdd0/game/", 15))
			...
			int n=0;
			s= ((char *) argv[0])+15;
			while(*s!=0 && *s!='/' && n<14) {app_path[n]=*s++; n++;} app_path[n]=0;
			}
	...
	{
		cellFsMkdir(app_homedir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		sprintf(app_usrdir, "/dev_hdd0/game/%s/USRDIR",app_path);
        ...
        sprintf(app_usrdir_data, "%s/DATA", app_usrdir);
	...
	if(!exist(disclaimer) || exist(string1))
	{
			...
			sprintf(string3x, "You are solely responsible for complying with the applicable laws in your country and you must cease using this software should your actions during multiMAN(SoniMan) operation lead to or may lead to infringement or violation of the rights of the respective content copyright owners.\n\nThis SOFTWARE is not licensed, approved or endorsed by \"Sony Computer Entertainment, Inc.\" (SCEI), SNEI, SEN or any other party.\n\nDo you understand and accept this binding agreement?");
	...
	else
	{
		...
		change_param_sfo_field( string1, (char*)"TITLE", (char*) "SonicMan-Cobra-ODE");
	}
...
//====================================================================
	...
	sprintf(avchdBG, "%s/BOOT.PNG", app_usrdir_data);
	if(!no_bootscreen)
	{
		load_texture(text_BOOT, avchdBG, 1920);
		sprintf(string1, "%s/GLO.PNG", app_usrdir_data);
		if(exist(string1))
		{
...
/*	c_firmware = (float) get_system_version();
	...
	cellFsMkdir(covers_dir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);

	sprintf(covers_retro, "%s/covers_retro", app_usrdir);
	cellFsMkdir(covers_retro, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);

	sprintf(themes_dir, "%s/themes", app_usrdir);
	cellFsMkdir(themes_dir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
	sprintf(themes_web_dir, "%s/themes_web", app_usrdir);
	cellFsMkdir(themes_web_dir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);

	sprintf(string1, "%s/lang", app_usrdir);
	cellFsMkdir(string1, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
	...
	sprintf(userBG,		"%s/PICBG.JPG", app_usrdir_data);

	sprintf(xmbicons,	"%s/XMB.PNG", app_usrdir_data);
	sprintf(xmbicons2,	"%s/XMB2.PNG", app_usrdir_data);
	sprintf(xmbdevs,	"%s/XMB64.PNG", app_usrdir_data);
	sprintf(xmbbg,		"%s/XMBBG.PNG", app_usrdir_data);
	sprintf(xmbbg_user_jpg,		"%s/XMBBGU.JPG", app_usrdir_data);
	sprintf(xmbbg_user_png,		"%s/XMBBGU.PNG", app_usrdir_data);

	sprintf(filename, "%s/XMBR.PNG", app_usrdir_data); remove(filename);

	sprintf(filename, "%s/DOX.PNG", app_usrdir_data);
	load_texture(text_DOX, filename, dox_width);

	sprintf(playBGR, "%s/PICPA.PNG", app_usrdir_data);
	sprintf(blankBG, "%s/ICON0.PNG", app_homedir);
	sprintf(filename, "%s/ICON0.PNG", app_usrdir_data);
	if(exist(filename)) file_copy(filename, blankBG, 0);
	...
	sprintf(helpNAV, "%s/NAV.JPG", app_usrdir_data);
	sprintf(helpMME, "%s/SOLAR.SELF", app_usrdir);

	sprintf(ps2png, "%s/PS2.JPG", app_usrdir_data);
	sprintf(dvdpng, "%s/DVD.JPG", app_usrdir_data);

	sprintf(avchdIN, "%s/AVCIN.DAT", app_usrdir);
	sprintf(avchdMV, "%s/AVCMV.DAT", app_usrdir);

	sprintf(iconHDD, "%s/HDD.JPG", app_usrdir_data);
	sprintf(iconUSB, "%s/USB.JPG", app_usrdir_data);
	sprintf(iconBLU, "%s/BLU.JPG", app_usrdir_data);
	sprintf(iconNET, "%s/NET.JPG", app_usrdir_data);
	sprintf(iconOFF, "%s/OFF.JPG", app_usrdir_data);

	sprintf(iconCFC, "%s/CFC.JPG", app_usrdir_data);
	sprintf(iconSDC, "%s/SDC.JPG", app_usrdir_data);
	sprintf(iconMSC, "%s/MSC.JPG", app_usrdir_data);
	sprintf(playBG,  "%s/FMS.PNG", app_usrdir_data);
	sprintf(legend, "%s/LEGEND2.PNG", app_usrdir_data);

	sprintf(versionUP,  "%s/VERSION.DAT", app_usrdir);

	sprintf(filename,  "%s/DROPS.PNG", app_usrdir_data);
	...
	cellFsMkdir(filename, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
	...
	if(!dir)
		cellFsMkdir(ini_hdd_dir, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); else closedir(dir);
	...
	sys_ppu_thread_create( &download_thr_id, download_thread_entry,
						   ...
						   0, "SonicMan-Cobra-ODE_downqueue" );

	sys_ppu_thread_create( &misc_thr_id, misc_thread_entry,
						   NULL, misc_thr_prio, app_stack_size,
						   0, "SonicMan-Cobra-ODE_misc" );//SYS_PPU_THREAD_CREATE_JOINABLE
...
//					game_sel=0;
	...
	{
		sort_entries(menu_list, &max_menu_list );
		sprintf(avchdBG, "%s/AVCHD.JPG", app_usrdir_data);
		load_texture(text_bmpUBG, avchdBG, 1920);
...
rename_title:
		 ...
		 if((strstr(menu_list[game_sel].path, "/dev_hdd0")!=NULL || strstr(menu_list[game_sel].path, "/dev_usb")!=NULL) && strstr(menu_list[game_sel].content, "PS3")!=NULL)
		 {
			...
			if(osk_dialog!=0)
			{
				...
				{
					for(u8 n=0; n<(strlen(pin_result)-3); n++)
					{
					...
					for(u8 n=0; n<(strlen(pin_result)-2); n++)
					{
						// (R)  = C2 AE    | 28 52 29
...
from_fm:
		if(cover_mode == MODE_FILEMAN) {
			...
			sprintf(auraBG, "%s/AUR5.JPG", app_usrdir_data);
...
copy_title:
		...
		if(menu_list[game_sel].flags & 1) // is hdd0
           ...
           if(dialog_ret==1)
				...
				cellFsMkdir(name, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
		...
		if(fdevices & 1)
			{
				...
				if(dialog_ret==1)
				{
					...
					cellFsMkdir(hdd_folder, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
...
check_from_start2:
	...
	//game_sel=xmb[xmb_icon].member[xmb[xmb_icon].first].game_id;  add_xmb_member_cobra(xmbmem *_member, u16 *_size, char *_name, char *_subname,
	...
	{game_sel=xmb[xmb_icon].member[xmb[xmb_icon].first].iso_num; cobra_iso_run(&manager.iso[game_sel]); cobra_manager_eject(&manager); }
					...
					if(strstr(filename, "/dev_hdd0/")==NULL && strstr(filename, "/dev_bdvd/")==NULL && (verify_data==2 || (verify_data==1)) ) // && payload==1
					{
						...
						if(num_files_big<=10 && max_joined<10 && payload==1)
						{
							...
							if(reprocess==1)
							{
								...
								cellFsMkdir(s_tmp, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(s_tmp, CELL_FS_S_IFDIR | 0777);
								...
								cellFsMkdir(string1, S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR); cellFsChmod(string1, CELL_FS_S_IFDIR | 0777);
								char tb_name[512];
