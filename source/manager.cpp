/* COBRA manager parsing library. v1.7
      ...
      if (manager->num_iso < MAX_ISO)
      {
        ...
        if (f)
        {
          ...
          if (zero == 0)
          {
            ...
            fread (&len, 1, 1, f);
            manager->iso[manager->num_iso].filename = (char*)malloc (len + 1);
	    ...
	    else
	    {
          ...
          {
            manager->iso[manager->num_iso].filename = (char*)malloc (2049);
...
CobraDisc * cobra_disc_open (CobraManager *manager)
{
      ...
      manager->info.disc_lba != 0)
  {
    ...
    if (cellFsOpen (manager->disc_iso, SYS_O_RDONLY, &disc->fd, NULL, 0) == 0)
    {
      u64 read, pos;
      ...
      disc->sector = 0;
      cellFsRead(disc->fd, &disc->sector0, 2048, &read);
      cellFsLseek(disc->fd, 0, SEEK_SET, &pos);
    }
    else
    {
      free (disc);
...
u32 cobra_disc_write (CobraDisc *disc, s32 output)
{
  ...
  if (cellFsRead(disc->fd, disc->buffer, 2048 * 32, &read) == 0)
  {
    ...
    if (cellFsWrite(output, disc->buffer, read, &written) == 0)
      bytes = (u32) written;
    else
      bytes = 0;
  }
